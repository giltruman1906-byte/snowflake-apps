# ─────────────────────────────────────────────
# Snowflake Pulse — Run Detail
# ─────────────────────────────────────────────

import streamlit as st
from snowflake.snowpark.context import get_active_session

st.set_page_config(page_title="Run Detail | Pulse", page_icon="⚡", layout="wide")
...
if not run_info.empty:
    ...
    status_emoji = "✅" if r["RUN_STATUS"] == "SUCCEEDED" else "❌" if r["RUN_STATUS"] == "FAILED" else "⏭️"
    c1.metric("Status",       f'{status_emoji} {r["RUN_STATUS"]}')
    c2.metric("Duration (s)", f'{r["DURATION_SECONDS"]:.1f}' if r["DURATION_SECONDS"] else "N/A")
    c3.metric("Credits",      f'{r["ESTIMATED_CREDITS"]:.4f}' if r["ESTIMATED_CREDITS"] else "0")
...
if not queries_df.empty:
    queries_display = queries_df.rename(columns={
        "QUERY_ID": "Query ID",
        "QUERY_TYPE": "Type",
        "EXECUTION_STATUS": "Status",
        "TOTAL_ELAPSED_TIME_MS": "Time (ms)",
    ...
    st.dataframe(queries_display, use_container_width=True)
